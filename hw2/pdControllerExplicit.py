...
class PDControllerExplicitMultiDof(object):
    ...
    def computePD(self, bodyUniqueId, jointIndices, desiredPositions, desiredVelocities, kps, kds, maxForces, timeStep):
        ...
        for i in range(numJoints):
            ...
            jointPos = js[0]
            jointVel = js[1]
            #print("pos:{}".format(jointPos))
            #print("vel:{}".format(jointVel))
            q1 += jointPos
            ...
            if len(js[0]) == 1:
                ...
                qdotIndex += 1
            if len(js[0]) == 4:
                desiredPos = [desiredPositions[qIndex], desiredPositions[qIndex + 1], desiredPositions[qIndex + 2],
                ...
                qError.append(0)
                desiredVel = [desiredVelocities[qdotIndex], desiredVelocities[qdotIndex + 1],
...
class PDControllerExplicit(object):
    ...
    def computePD(self, bodyUniqueId, jointIndices, desiredPositions, desiredVelocities, kps, kds, maxForces):
        #numJoints = self._pb.getNumJoints(bodyUniqueId)
        jointStates = self._pb.getJointState(bodyUniqueId, jointIndices)
        q1 = []
        qdot1 = []
        #for i in range(numJoints):
